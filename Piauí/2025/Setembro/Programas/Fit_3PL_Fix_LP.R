...
base <- base %>% dplyr::filter(disciplina_descricao == 'LÍNGUA PORTUGUESA')

# 2.3) Elimina itens/colunas específicas (ex.: erro/campo duplicado)
#     OBS: ajuste os nomes conforme a realidade do banco
suppressWarnings({
  base <- base %>% dplyr::select(-dplyr::any_of(c("rpa_010","rp_010","rpa_019","rp_019"))) # O item 10 foi anulado e o item 19 eliminado (possível conflitono gabarito)
})
...
InfoItens<-read_xlsx(here('itens-comuns-setembro.xlsx'),sheet = 'LP-Setembro')

# Eliminando itens

InfoItens<- InfoItens %>% filter(Item!='rpa_010' & Item!='rpa_019')
...
pdf(file=paste(file.save,sep="","/ParItensE.pdf"))
par(mfrow=c(2,2))
plot(ea,cex=1.2,
...
pdf(file=paste(file.save,sep="","/TracosLatentesET.pdf"))
par(mfrow=c(2,2))
...
#
pdf(file=paste(file.save,sep="/TracosLatentesETPDI",".pdf"))
par(mfrow=c(1,1))
...
pdf(file=paste(file.save,sep="","/CCIPOE.pdf"))
for(i in 1:nI)
{
  par(mfrow=c(1,1))
...
pdf(file=paste(file.save, sep="", "/IICPOE.pdf"))
for (i in 1:nI) {
  par(mfrow=c(1,1))
...
pdf(file=paste(file.save,sep="","/PPE.pdf"))
par(mfrow=c(1,2))
...
pdf(file=paste(file.save,sep="","/PPET.pdf"))
par(mfrow=c(1,2))
...
write.xlsx(base,paste(file.save,sep="/BaseRespTheta_3EM_LP",".xlsx"),rowNames=T)
...
write.xlsx(freqTheta,paste(file.save,sep="/DistriAlunosClasseLP3serie",".xlsx"),rowNames=T)
...
write.xlsx(TabItens,paste(file.save,sep="/EstItens_3EM_LP",".xlsx"),rowNames=F)
...
write.xlsx(as.data.frame(med_resu),paste(file.save,sep="/ResumoTheta_3EM_LP",".xlsx"),rowNames=T)
...
write.xlsx(rbind(EstPop),paste(file.save,sep="/ResumoThetaIC_3EM_LP",".xlsx"),rowNames=T)
...
pdf(file=paste(file.save,sep="","/S_X2item.pdf"))

# X2
# resultML3P: estimativa dos parametros dos itens
pdf(file=paste(file.save,sep="","/X2item.pdf"))
par(mfrow=c(1,1))
...
#
# G2
pdf(file=paste(file.save,sep="","/G2item.pdf"))
par(mfrow=c(1,1))
...
# 
# PV_Q1
pdf(file=paste(file.save,sep="","/PV_Q1item.pdf"))
par(mfrow=c(1,1))
...
pdf(file=paste(file.save,sep="","/infititem.pdf"))
par(mfrow=c(1,1))
...
#
# Outfit
pdf(file=paste(file.save,sep="","/outfititem.pdf"))
par(mfrow=c(1,1))
plot(infout[,2],
