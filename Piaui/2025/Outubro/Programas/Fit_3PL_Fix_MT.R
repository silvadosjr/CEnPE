...
setwd('C:/Users/rober/OneDrive/Documentos/CEnPE/Piauí/Outubro/')
...
base <- readRDS(here('df_piaui_outubro.RDS'))
...
suppressWarnings({
  base <- base %>% dplyr::select(-dplyr::any_of(c("rpa_022","rp_022"))) # 
})
...
InfoItens<-read_xlsx(here('itens-comuns-out.xlsx'),sheet = 'MT-Outubro')
...
write.xlsx(base,paste(file.save,sep="/BaseRespTheta_3EM_MT",".xlsx"),rowNames=T)
...
write.xlsx(freqTheta,paste(file.save,sep="/DistriAlunosClasseMT3serie",".xlsx"),rowNames=T)
...
write.xlsx(TabItens,paste(file.save,sep="/EstItens_3EM_MT",".xlsx"),rowNames=F)
...
write.xlsx(as.data.frame(med_resu),paste(file.save,sep="/ResumoTheta_3EM_MT",".xlsx"),rowNames=T)
...
write.xlsx(rbind(EstPop),paste(file.save,sep="/ResumoThetaIC_3EM_MT",".xlsx"),rowNames=T)
