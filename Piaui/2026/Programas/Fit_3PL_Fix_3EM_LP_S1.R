...
base <- readRDS(file.path(dados_dir, "df_1ª_SIMULA_3ª_SÉRIE.RDS"))
...
# suppressWarnings({
#   base <- base %>%
#     dplyr::select(-dplyr::any_of(c("rpa_008", "rp_008","rpa_012", "rp_012","rpa_020", "rp_020")))
# })
...
InfoItens<-read_xlsx(file.path(dados_dir,'gabarito_3serie_lp 1ª SIMULA.xlsx'),sheet = 'LP-SimI')

# Eliminando itens

#InfoItens<- InfoItens %>% filter(Item!='rpa_008' & Item!='rpa_012'& Item!='rpa_020')
...
write.xlsx(base,paste(result_dir,sep="/BaseRespTheta_3EM_LP_S1",".xlsx"),rowNames=T)
...
write.xlsx(freqTheta,paste(result_dir,sep="/DistriAlunosClasseLP3serie_S1",".xlsx"),rowNames=T)
...
write.xlsx(TabItens,paste(result_dir,sep="/EstItens_3EM_LP_S1",".xlsx"),rowNames=F)
...
write.xlsx(as.data.frame(med_resu),paste(result_dir,sep="/ResumoTheta_3EM_LP_S1",".xlsx"),rowNames=T)
...
write.xlsx(rbind(EstPop),paste(result_dir,sep="/ResumoThetaIC_3EM_LP_S1",".xlsx"),rowNames=T)
