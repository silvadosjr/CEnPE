...
# 2.3) Elimina itens/colunas específicas, se necessário
 suppressWarnings({
   base <- base %>%
     dplyr::select(-dplyr::any_of(c("rpa_014", "rp_014")))
 })
...
InfoItens<-read_xlsx(file.path(dados_dir,'gabarito_3serie_mt 2ª SIMULA.xlsx'),sheet = 'MT-SimII')

# Eliminando itens

InfoItens<- InfoItens %>% filter(Item!='rpa_014')
...
write.xlsx(base,paste(result_dir,sep="/BaseRespTheta_3EM_MT_S2",".xlsx"),rowNames=T)
...
write.xlsx(freqTheta,paste(result_dir,sep="/DistriAlunosClasseMT3serie_S2",".xlsx"),rowNames=T)
...
write.xlsx(TabItens,paste(result_dir,sep="/EstItens_3EM_MT_S2",".xlsx"),rowNames=F)
...
write.xlsx(as.data.frame(med_resu),paste(result_dir,sep="/ResumoTheta_3EM_MT_S2",".xlsx"),rowNames=T)
...
write.xlsx(rbind(EstPop),paste(result_dir,sep="/ResumoThetaIC_3EM_MT_S2",".xlsx"),rowNames=T)
