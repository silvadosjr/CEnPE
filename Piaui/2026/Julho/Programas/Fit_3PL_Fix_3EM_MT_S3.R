...
base <- readRDS(file.path(dados_dir, "df_3ª_SIMULA_3ª_SÉRIE.RDS"))
...
InfoItens<-read_xlsx(file.path(dados_dir,'gabarito_3serie_mt 3ª SIMULA.xlsx'),sheet = 'MT_3EM_S3')

# Eliminando itens

InfoItens<- InfoItens %>% filter(Item!='rpa_025' & Item!='rpa_006')
...
write.xlsx(base,paste(result_dir,sep="/BaseRespTheta_3EM_MT_S3",".xlsx"),rowNames=T)
...
write.xlsx(freqTheta,paste(result_dir,sep="/DistriAlunosClasseMT3serie_S3",".xlsx"),rowNames=T)
...
write.xlsx(TabItens,paste(result_dir,sep="/EstItens_3EM_MT_S3",".xlsx"),rowNames=F)
...
write.xlsx(as.data.frame(med_resu),paste(result_dir,sep="/ResumoTheta_3EM_MT_S3",".xlsx"),rowNames=T)
...
write.xlsx(rbind(EstPop),paste(result_dir,sep="/ResumoThetaIC_3EM_MT_S3",".xlsx"),rowNames=T)
