...
proj_dir <- file.path(root_dir, "Piaui", "2026","Agosto")
...
base <- readRDS(file.path(dados_dir, "df_4ª_SIMULA_3ª_SÉRIE.RDS"))
...
suppressWarnings({
  base <- base %>%
    dplyr::select(-dplyr::any_of(c("rpa_002", "rp_002"))) # Anulado 
})
...
InfoItens<-read_xlsx(file.path(dados_dir,'gabarito_3serie_mt 4ª SIMULA.xlsx'),sheet = 'MT_3EM_S4')

# Eliminando itens

InfoItens<- InfoItens %>% filter(Item!='rpa_002')
...
write.xlsx(base,paste(result_dir,sep="/BaseRespTheta_3EM_MT_S4",".xlsx"),rowNames=T)
...
write.xlsx(freqTheta,paste(result_dir,sep="/DistriAlunosClasseMT3serie_S4",".xlsx"),rowNames=T)
...
write.xlsx(TabItens,paste(result_dir,sep="/EstItens_3EM_MT_S4",".xlsx"),rowNames=F)
...
write.xlsx(as.data.frame(med_resu),paste(result_dir,sep="/ResumoTheta_3EM_MT_S4",".xlsx"),rowNames=T)
...
write.xlsx(rbind(EstPop),paste(result_dir,sep="/ResumoThetaIC_3EM_MT_S4",".xlsx"),rowNames=T)
