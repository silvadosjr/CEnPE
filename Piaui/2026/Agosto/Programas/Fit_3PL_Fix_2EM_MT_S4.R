...
base <- readRDS(file.path(dados_dir, "df_4ª_SIMULA_2ª_SÉRIE.RDS"))
...
suppressWarnings({
  base <- base %>%
    dplyr::select(-dplyr::any_of(c("rpa_024", "rp_024"))) # Possível conflito de gabarito
})
...
InfoItens<-read_xlsx(file.path(dados_dir,'gabarito_2serie_mt 4ª SIMULA.xlsx'),sheet = 'MT_2EM_S4')
...
write.xlsx(base,paste(result_dir,sep="/BaseRespTheta_2EM_MT_S4",".xlsx"),rowNames=T)
...
write.xlsx(freqTheta,paste(result_dir,sep="/DistriAlunosClasseMT2serie_S4",".xlsx"),rowNames=T)
...
write.xlsx(TabItens,paste(result_dir,sep="/EstItens_2EM_MT_S4",".xlsx"),rowNames=F)
...
write.xlsx(as.data.frame(med_resu),paste(result_dir,sep="/ResumoTheta_2EM_MT_S4",".xlsx"),rowNames=T)
...
write.xlsx(rbind(EstPop),paste(result_dir,sep="/ResumoThetaIC_2EM_MT_S4",".xlsx"),rowNames=T)
